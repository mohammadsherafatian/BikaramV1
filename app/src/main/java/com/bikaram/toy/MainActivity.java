...
public class MainActivity extends Activity implements BoredomView.Host {
    ...
    protected void onResume() {
        super.onResume();
        if (gameView != null) gameView.startSensors();
    }

    @Override
    protected void onPause() {
        if (gameView != null) gameView.stopSensors();
        super.onPause();
    }
    ...
    public void shareSnapshot(String text) {
        try {
            Bitmap bitmap = Bitmap.createBitmap(gameView.getWidth(), gameView.getHeight(), Bitmap.Config.ARGB_8888);
            Canvas canvas = new Canvas(bitmap);
            gameView.draw(canvas);

            ContentValues values = new ContentValues();
            values.put(MediaStore.Images.Media.DISPLAY_NAME, "bikaram-" + System.currentTimeMillis() + ".png");
